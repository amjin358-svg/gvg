...
export function SiteHeader() {
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          isHome ? "border-b border-white/10 bg-transparent" : "border-b border-[var(--color-line)] bg-white",
        )}
      >
        <Container className="flex h-16 items-center gap-4 lg:h-[4.5rem]">
          <Link href="/" className="flex shrink-0 items-center gap-2.5">
            ...
            <img
              ...
              alt="GVG"
              className={cn("h-10 w-10", isHome && "brightness-110")}
            />
            <span className="leading-tight">
              <span
                className={cn(
                  "block font-[family-name:var(--font-display)] text-base font-semibold",
                  isHome ? "text-white" : "text-[var(--color-navy)]",
                )}
              >
                {BRAND.shortName}{" "}
                <span
                  className={cn(
                    ...
                    isHome ? "text-[var(--color-gold)]" : "text-[var(--color-gold-strong)]",
                  )}
                >
                  GLOBAL VISTA GROUP
                </span>
          ...
          <div
            className={cn(
              "ml-auto hidden items-center gap-4 text-sm lg:flex",
              isHome ? "text-white/85" : "text-[var(--color-muted)]",
            )}
          >
            <Link
              ...
              className={cn(
                ...
                isHome ? "hover:text-white" : "hover:text-[var(--color-navy)]",
              )}
            >
            ...
            >
              ...
              <span
                className={cn(
                  "absolute -right-2 -top-2 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] font-bold",
                  isHome
          ...
          <button
            type="button"
            className={cn(
              "ml-auto inline-flex h-10 w-10 items-center justify-center rounded lg:hidden",
              isHome ? "text-white" : "text-[var(--color-navy)]",
            )}
            aria-expanded={open}
      ...
      {/* Tier 3 — primary modules (compact on home) */}
      <nav
        className={cn(
          "hidden lg:block",
          isHome
            ? "border-b border-white/10 bg-transparent"
            : "border-b border-[var(--color-line)] bg-white",
        )}
        aria-label="主選單"
      >
                ...
                {active ? (
                  <span
                    className={cn(
                      ...
                      isHome ? "bg-[var(--color-gold)]" : "bg-[var(--color-navy)]",
                    )}
                  />
                ) : null}
              </Link>
      ...
      <div
        ...
        className={cn(
          "lg:hidden",
          isHome
            ? "border-b border-white/10 bg-[#05080f]/95"
            : "border-b border-[var(--color-line)] bg-white",
          open ? "block" : "hidden",
        )}
      >
        <Container className="flex flex-col gap-1 py-4">
          ...
          {[...PRIMARY_NAV, ...UTILITY_NAV].map((item) => (
            <Link
              ...
              href={item.href}
              className={cn(
                "rounded px-2 py-2.5 text-sm font-medium",
                isHome
          ...
          <Link
            ...
            className={cn(
              "rounded px-2 py-2.5 text-sm font-medium",
              isHome ? "text-white" : "text-[var(--color-ink)]",
            )}
            onClick={() => setOpen(false)}
          >
            登入 / 註冊
