...
import { BUSINESS_NAV } from "@/frontend/data/business/content";
      ...
      <Container className="grid gap-8 py-8 lg:grid-cols-[280px_1fr]">
        <aside className="h-fit rounded-xl border border-[var(--color-line)] bg-white p-4">
          <div className="mb-4 flex items-center gap-2 px-2">
            <BriefcaseBusiness className="h-4 w-4 text-[var(--color-navy)]" />
            <div>
              <p className="text-sm font-semibold text-[var(--color-navy)]">Business Center</p>
              <p className="text-xs text-[var(--color-muted)]">企業資訊中心</p>
            </div>
          </div>
          ...
          <div className="mt-6 rounded-lg bg-[var(--color-mist)] p-4 text-xs text-[var(--color-muted)]">
            文件來源：`docs/business/` 與產品路線圖。對外分享前請補齊法律與財務欄位。
          </div>
        </aside>

        <div>
          <p className="text-xs text-[var(--color-muted)]">首頁 &gt; 企業資訊 &gt; {titleZh}</p>
